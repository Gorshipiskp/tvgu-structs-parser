...
Асинхронный парсер структуры Тверского государственного университета (ТвГУ), собирающий и нормализующий информацию о
факультетах, институтах и кафедрах из нескольких публичных источников.
...
В университете **нет единого формального API**, содержащего всю необходимую информацию, поэтому парсер агрегирует данные
...
Выкладываю проект, чтобы у студентов и разработчиков была возможность получать **целостные и согласованные данные о
структуре университета** для своих сервисов и приложений.
...
Агрегация этих источников позволяет получить наиболее подробные сведения о каждой структуре _(либо я не знаю нужного
эндпоинта)_.
...
- При несовпадении данных между источниками парсер **намеренно падает с ошибкой**, чтобы избежать экспорта
...
Используйте функцию `get_all_tvgu_structs()` – просто вызывайте её _(не забудьте `await`)_ и получите список всех
структур ТвГУ
